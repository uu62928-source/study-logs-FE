...
```ts
// Good: feature内部
import { calculateTotalStudyMinutes } from '../domain/studyLog'

// Bad: 自分自身の公開窓口へ戻る
import { calculateTotalStudyMinutes } from '@/features/study-log'
```
...
2. 共通部分が特定featureの業務知識を持たないなら、`shared`へ置くべきか
3. `app`などの上位層で組み合わせられないか

同じ業務ルールを複数featureが必要とする場合は、形が似ているという理由だけで`shared`へ移さず、featureの境界自体を見直す。
...
この矢印は処理やデータの流れではなく、コード上の依存方向を表す。
