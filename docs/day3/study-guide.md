...
- [ ] 画面で扱う状態をclient、server、form、URL、derivedに分類した
- [ ] ユーザー操作と外部から起きるイベントを列挙した
- [ ] `useState`または`useReducer`を選ぶ理由を説明できる
- [ ] reducerのstateとeventを型で定義した
- [ ] Domainロジック、状態遷移、副作用の置き場所を決めた
- [ ] custom hookがUIへ公開する値と操作を決めた
- [ ] localStorageの保存形式と異常データの扱いを決めた
- [ ] URLを情報源にする検索条件を決めた
- [ ] Contextが本当に必要か確認した
- [ ] 保存失敗や競合が起きた場合の振る舞いを決めた

## 完了時のチェックリスト

- [ ] 学習項目、学習時間、日付を追加できる
- [ ] 合計学習時間が元の一覧から正しく計算される
- [ ] 編集と削除が状態遷移として実装されている
- [ ] フィルタと並び替えが動作する
- [ ] 検索条件がURLへ反映され、再読み込み後も復元される
- [ ] 学習ログがlocalStorageへ保存され、再読み込み後も復元される
- [ ] localStorageの空データ、不正なJSON、不正な値を安全に扱える
- [ ] reducerまたは状態遷移の純粋関数を単体テストした
- [ ] 保存と復元をstorage adapterのテストで確認した
- [ ] 不要なContextと重複stateがない
- [ ] `npm run lint`、`npm run test`、`npm run build`が成功する
- [ ] 状態管理の選択理由と残課題を振り返りへ記録した
